...
import { existsSync, lstatSync, mkdirSync, readFileSync, realpathSync, symlinkSync, readdirSync } from 'node:fs';
...
function link(src,dst,root) {
  ...
  if (present(dst)) throw Error('destination already exists'); mkdirSync(dirname(dst),{recursive:true}); symlinkSync(src,dst,ss.isDirectory()?'junction':'file');
  return {source:src,destination:dst,digest:digest(src),access:'read-only',materialization:'symlink',sourceType:ss.isDirectory()?'directory':'file'};
}
